...
const types = [PostcardType.MUSHROOM, PostcardType.FLOWER, PostcardType.EXPLORATION];
...
async function main() {
  ...
  try {
    ...
    const rows = Array.from({ length: count }, (_, index) => {
      ...
      const createdAt = new Date(now - Math.floor(Math.random() * 18 * 24 * 60 * 60 * 1000));
      ...
      return {
        ...
        locationStatus: aiGenerated ? LocationStatus.AUTO : LocationStatus.MANUAL,
    ...
    const result = await prisma.postcard.createMany({ data: rows });
    console.log(`Seeded ${result.count} postcards for ${email}`);
  } finally {
